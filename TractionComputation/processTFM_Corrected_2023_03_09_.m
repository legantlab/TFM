...
fileName = 'U:\Max\2023_03_03_IA32CellsInChannels\Timelapse_01\SwellingControl\SwellingControlExpandedSolverDeck.inp';
...
%% Load displacements
load("U:\Max\2023_03_03_IA32CellsInChannels\Timelapse_01\SwellingControl\Swelling Control.mat")
...
polymodelx = polyfitn(CENTROIDSR(1:1:end,2:4),allDisps(1:1:end,1),11);
...
figure
subplot(1,2,1)
quiver3(CENTROIDSR(:,2)*.1268,CENTROIDSR(:,3)*.1268,CENTROIDSR(:,4)*0.8,...
...
beadPos(:,1:2) = beadPos(:,1:2)*.1268;
beadPos(:,3) = beadPos(:,3)*.8;
...
for kk=1:length(matches)
    ...
    curDisp(:,1:2) = curDisp(:,1:2)*.1268;
    curDisp(:,4:5) = curDisp(:,4:5)*.1268;
    curDisp(:,3) = curDisp(:,3)*.8;
    curDisp(:,6) = curDisp(:,6)*.8;
...
nodalSolutionsData = h5read('U:\Max\2023_02_26_IA32CellsInChannels\S6\S6_Fixed_Denser_Loaded_Thermal_Again_LoadOnly.h5','/OPTISTRUCT/RESULT/NODAL/DISPLACEMENT'); %This takes a few minutes at least. Can't do string as it becomes enormous. 
...
subcaseIDs = 1:(length(elemCents2D(:,1))*3 + 1);
...
%% Compute tractions
%Remove the last row of  which is the thermal drift I think
force_vector={};
for kk=1:length(beadDispsCleaned)
    ...
    force_vector{kk}=[elemCents2D,x_lambda(1:3:end-1),x_lambda(2:3:end-1),x_lambda(3:3:end-1)];
end
...
for i = 1:length(force_vector)
    %curCellImage = imread
    figure
    quiver(elemCents2D(:,1),elemCents2D(:,2),force_vector{i}(:,4),force_vector{i}(:,5),1)
    set(gca,'visible','off')
    ax= gca;
    axis equal
    exportgraphics(ax,['U:\Max\2023_02_26_IA32CellsInChannels\S6\ForceMaps\', 'TractionForces_',num2str(i),'.png'],'Resolution',600)
