function [beadDisps] = interpDispsDisplacements(matches,appliedNodeList)
...
%}   
    ...
    for ii=1:length(matches)
        ...
        sI1 = scatteredInterpolant(matches{ii}(:,1),matches{ii}(:,2),matches{ii}(:,3),curDisp(:,1),'linear', 'none');%'linear','none'%,'nearest', 'nearest');
        sI2 = scatteredInterpolant(matches{ii}(:,1),matches{ii}(:,2),matches{ii}(:,3),curDisp(:,2),'linear', 'none');%,'nearest', 'nearest');
        sI3 = scatteredInterpolant(matches{ii}(:,1),matches{ii}(:,2),matches{ii}(:,3),curDisp(:,3),'linear', 'none');%,'nearest', 'nearest');
