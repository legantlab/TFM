function [x] = locateParticles(I, threshold_multi,bandpass_size)
...
[IBP]=bpass3dMB(I, [.5,.5,.5], bandpass_size,[0,0]); %Bandpass filter - Can tune parameters for optimal fitting
minDist = mean(bandpass_size);           % minimum distance between two peaks for local maximum detection

threshold = prctile(IBP(:),90)*threshold_multi;   %threold set to be threshold_multi * 90
...
%voxel2um = [0.1917, 0.1917, 0.5]; %put in coversion for your system
%x = [x.*voxel2um];
end
