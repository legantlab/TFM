function [matchIndex]=matchFeatureVector_2010_09_09(refVectors,defVectors,numVectorsRef,numVectorsDef,numNeighbors,tol)
...
for i=1:length(refVectors)
    for j=1:numNeighbors
        for k=1:numPerms
            residual(k,j)=norm(refVectors(i,:)-defVectors(i,(j-1)*3*numVectorsDef+PV(k,:)));
        end
    end
