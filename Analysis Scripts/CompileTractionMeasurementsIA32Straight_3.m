...
%files = [F00; F01; F02; F03; F09; F12; F13; F14;F15;F16];
files = [F01; F02; F09; F13; F14; F13_01; F14_01];
...
maskDir = [MasksF01;MasksF02;MasksF09;MasksF13;MasksF14; MasksF13_01; MasksF14_01];
...
DNAF13_01 = "S:\Max\2024_03_21_MicrochannelTFM\StraightOvernight\2024-03-21\tiffs\F13\DNA_Masks";
DNAF14_01 = "S:\Max\2024_03_21_MicrochannelTFM\StraightOvernight\2024-03-21\tiffs\F14\DNA_Masks";

%DNADir = [DNAF00; DNAF01; DNAF02; DNAF03; DNAF09; DNAF12; DNAF13; DNAF14; DNAF15; DNAF16];
DNADir = [DNAF01; DNAF02; DNAF09; DNAF13; DNAF14; DNAF13_01; DNAF14_01];
...
%NCMz = zeros(length(traction_vector),1);
    ...
    %Pick random point in channel to serve as the "center"
%     channelCenter = [(min(curNucCentroids(:,1)) + max(curNucCentroids(:,1)))/2 ,...
%         (min(curNucCentroids(:,2)) + max(curNucCentroids(:,2)))/2];
    channelCenter = curNucCentroids(round(rand(1)*length(curNucCentroids)/4)...
        +round(length(curNucCentroids)/2),:);
...
for i = 1:length(strainEnergy)
    ...
    elemCentroids(:,1) = round(elemCentroids(:,1)/.199 - 1 + imgSize(2)/2);
    elemCentroids(:,2) = round(elemCentroids(:,2)/.199 - 1 + imgSize(1)/2);
    ...
    if isempty(maskStats)
        maskCentroid = [nan,nan];
        nucAspect(i) = nan;
        solidity(i) = nan;
        orientation(i) = nan;
        perimeter(i) = nan;
        distCenterHullmin = nan;
        nucArea(i) =nan;
    else
...
sampleGrid = [50]; %linspace(-50,50,21);
figure
hold on 
...
for i = 1:length(compiledSE)
    ...
    sortIdx = sortIdx(~isnan(sortDisps));
    sortDisps = sortDisps(~isnan(sortDisps));
    ...
    sortHullIdx = sortHullIdx(~isnan(sortHullDisps));
    sortHullDisps = sortHullDisps(~isnan(sortHullDisps));
...
%load('StraightNucleiSegment.mat')
