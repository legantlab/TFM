...
folder = 'T:\Max\2023-06-22\Tiffs\GreatMovies\ON2_F22\Cropped\DNA';
outFold = 'T:\Max\2023-06-22\Tiffs\GreatMovies\ON2_F22\Cropped\DNAseg';
...
for j = 1:length(axisRatioStore)
    curProp = storeProps{j};
    axisRatioStore(j) = curProp.MajorAxisLength/curProp.MinorAxisLength;
    areaStore(j) = curProp.Area;
end
figure
scatter(linspace(1,length(tiffs),length(tiffs)),axisRatioStore)
%xlim([0 10]);
xlabel('Frame')
ylabel('Aspect Ratio')

figure
scatter(linspace(1,length(tiffs),length(tiffs)),areaStore)
%xlim([0 10]);
xlabel('Frame')
ylabel('Area (pix)')
