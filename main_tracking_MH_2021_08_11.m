...
save([analysisStem dataFileName '.mat'], 'matches', 'displacements','initLocations','finLocations','tiffs','driftStore');

%%
originalBeadLocations = x(2:end);

fViewGUI = finalViewGUI_OriginalLocations(displacements,matches,tiffs,driftStore,originalBeadLocations,displacementsWithDrift);
...
fViewGUI.delete
%% Output displacement vectors to Amira
%We probably need to change the units on the beads, scale to our microscope
%pixel sizes. Nikon 60x has 0.1083329722234259 um pixels, or ~108.3 nm
%pixels
text = buildAmiraMesh(test(:,1:3), test2);
fid = fopen(['\\LEGANTLABSERVER\LegantLab_DG1_VD1\Max\2021_09_24_F127GelsJR20\New folder\Processed Data\Series5\'...
    'AmiraTetraVectorsDisplacements2021_10_01' '.am'],'wt');
fprintf(fid, text);
fclose(fid);

%% Display Bead Bounding Box Info
