...
allCents=cell2mat(matchesR);
allDisps=cell2mat(displacementsR);
...
dispsC={};
for ii=1:51
    index=find(time_index==ii)
[numBeads,~]=size(matchesR{index}(:,1));
CENTROIDSR=[[1:numBeads]',matchesR{index}(:,4:6)];
...
displacementsR_dedrift{index}=[displacementsR{index}(:,1)-dispsC{index}(:,1),displacementsR{index}(:,2)-dispsC{index}(:,2),displacementsR{index}(:,3)-dispsC{index}(:,3)];

h = figure;
filename = 'testAnimated.gif';
scale=0.75;
quiverC3D(CENTROIDSR(:,2)*.108,CENTROIDSR(:,3)*.108,CENTROIDSR(:,4)*.3,displacementsR_dedrift{index}(:,1)*scale,displacementsR_dedrift{index}(:,2)*scale,displacementsR_dedrift{index}(:,3)*scale,0)
axis equal
set(gca,'Color','k')
