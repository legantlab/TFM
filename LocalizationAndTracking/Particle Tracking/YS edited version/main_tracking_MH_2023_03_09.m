...
% Track Particles
[x, track] = funTPT_V2(fileInfo, beadParam, tptParam, isCZI);
% save([analysisStem dataFileName '_track.mat'], 'x', S'track', 'fileInfo');
% Account for pixel to unit conversion here:
for i = 1:length(x)
    curX = x{i};
    curX{1}(:,1) = size_x.*curX{1}(:,1);
    curX{1}(:,2) = size_y.*curX{1}(:,2);
    curX{1}(:,3) = size_z.*curX{1}(:,3); 
    x{i} = curX;
end
...
% Manual area selection for computing drift correction
% figure
...
%     % Accounts for pixel to units converstion 
...
%     matches_timet(:,[3 6]) = size_z*matches_timet(:,[3 6]);
    ...
    % dcBeads = find(matches_timet(:,1) >= dcArea(1) & matches_timet(:,1) <= dcArea(1) + dcArea(3) & ...
    %     matches_timet(:,2) >= dcArea(2) & matches_timet(:,2) <= dcArea(2) + dcArea(4));

    %store the drift values for image registration
    % 
    % driftx = mean(matches_timet(dcBeads,4)-matches_timet(dcBeads,1));
    % drifty = mean(matches_timet(dcBeads,5)-matches_timet(dcBeads,2));
    % driftz = mean(matches_timet(dcBeads,6)-matches_timet(dcBeads,3));

    driftx = median(matches_timet(:,4)-matches_timet(:,1));
    drifty = median(matches_timet(:,5)-matches_timet(:,2));
    driftz = median(matches_timet(:,6)-matches_timet(:,3));
    ...
    %Translation
    matches_timet(:,4)=matches_timet(:,4)-median(matches_timet(:,4)-matches_timet(:,1));
    matches_timet(:,5)=matches_timet(:,5)-median(matches_timet(:,5)-matches_timet(:,2));
    matches_timet(:,6)=matches_timet(:,6)-median(matches_timet(:,6)-matches_timet(:,3));

%     matches_timet(:,4)=matches_timet(:,4)-driftx;
%     matches_timet(:,5)=matches_timet(:,5)-drifty;
%     matches_timet(:,6)=matches_timet(:,6)-driftz;
...
save([analysisStem '\' dataFileName 'GUIvariables' '.mat'], 'matches', 'displacements','tiffs','driftStore');

%%
tempMatches = matches;
for i = 1:length(tempMatches)
    tempMatches{i}(:,1:2) = tempMatches{i}(:,1:2)/size_x;
    tempMatches{i}(:,3) = tempMatches{i}(:,3)/size_z;
    tempMatches{i}(:,4:5) = tempMatches{i}(:,4:5)/size_x;
    tempMatches{i}(:,6) = tempMatches{i}(:,6)/size_z;
end
fViewGUI = finalViewGUI(displacements,tempMatches,tiffs,driftStore);
...
% figure(5)
% XYScaleCorrect = 0.120;
% %Ifuse = imfuse(sum(Ipost,3)>5000, sum(Ipre,3)>5000,'blend','Scaling','independent');
% Ifuse = imfuse(Ipost(:,:,70), Ipre(:,:,70),'blend');
% imshow(Ipre(:,:,78))
% caxis([5 20])
% hold on
% scatter(matches{1}(:,1)/XYScaleCorrect, matches{1}(:,2)/XYScaleCorrect, 200, 'r.')
% scatter(matches{1}(:,4)/XYScaleCorrect, matches{1}(:,5)/XYScaleCorrect, 200, 'g.')
% scatter(x{1}{1}(track{1}{1}==0,1), x{1}{1}(track{1}{1}==0,2), 'r')
...
%% Output Raw displacements as Amira Mesh?

% for i = 1:length(matches)
%     i
%     curPos = matches{i}(:,4:6);
%     curDisp = displacements{i};
%     %Move back positions using trans
%     
%     curPos(:,1) = curPos(:,1) + 1.2 + trans(1)*.199;
%     curPos(:,2) = curPos(:,2) - 0.5 + trans(2)*.199;
%     curPos(:,3) = curPos(:,3) + 0 + trans(3)*.8;
%     curPos = rotate_3D(curPos','x',1*trans(4))';
%     curPos = rotate_3D(curPos','z',1*trans(6))';
% 
%     text=buildAmiraMesh(curDisp,curPos);
%     fid=fopen(['T:\Max\2023-06-22\Tiffs\GreatMovies\ON2_F18\MoreCropped\rawDisplacements\t', sprintf( '%02d', i ), 'disps.am'],'wt');
%     fprintf(fid,text);
%     fclose(fid);
% 
% end