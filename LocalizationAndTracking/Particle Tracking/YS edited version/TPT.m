...
while iterCrit == 1
    ...
    %This might be where the problem is once we change the image reference
    %- MH 9/11/2023
    %Create grid for subset
    sGrid = cell(3,1);
    sGrid{1} = min(x0(:,1)):sSize(end):max(x0(:,1));
    sGrid{2} = min(x0(:,2)):sSize(end):max(x0(:,2));
    sGrid{3} = min(x0(:,3)):sSize(end):max(x0(:,3));

    % 
    %  sGrid = cell(3,1);
    % sGrid{1} = 1:sSize(end):sizeI(2);
    % sGrid{2} = 1:sSize(end):sizeI(1);
    % sGrid{3} = 1:sSize(end):sizeI(3);
    ...
    for i = 1 : nSSgrid          % Parallel computing
        ...
        %disp(['index = ' num2str(i) '; sPt = [' num2str(sPt(1)) ' ' num2str(sPt(2)) ' ' num2str(sPt(3)) ']; length(ssMatch) = ' num2str(length(ssMatch))])
    ...
    fprintf('    Elapsed time (Iteration %d): %0.4f secs   Particles tracked: %0.2f   Subset Size = %d\n',iter,toc(tIter),sum(track>0)/length(x1)*100,sSize(end-1));
    iter = iter+1;
