...
%             [corBeads3, ~] = alignOrigin(corBeads2,0); %Reallign to center after swelling correction? 
            ...
            %reapply translation matrix rather than doing a reallignment
            % newframe = x{t}{1};
            % newframe(:,1) = newframe(:,1) - trans(1);
            % newframe(:,2) = newframe(:,2) - trans(2);
            % newframe(:,3) = newframe(:,3) - trans(3);
            % newframe = rotate_3D(newframe','x',-1*trans(4))';
            % newframe = rotate_3D(newframe','z',-1*trans(6))';
...
%             x{2}{1} = [newframe(:,1)-2.5, newframe(:,2), newframe(:,3)];
            ...
            PC2reg = pctransform(PC2,tform);
            x{t}{j} = PC2reg.Location;      
            %correct position of frame using trans
            % newframe = x{t}{1};
            % newframe(:,1) = newframe(:,1) - trans(1);
            % newframe(:,2) = newframe(:,2) - trans(2);
            % newframe(:,3) = newframe(:,3) - trans(3);
            ...
            % % Do another round of bead matching for the remaining beads. 
            % ind = track{t-1}{1} ~= 0;
            % map = track{t-1}{1}(ind);
            ...
            % strainBeads = [(1:length(x{t-1}{1}))',x{t-1}{1}];
            % 
            % test = displacementCalc_recursive_2010_09_09(refBeads,strainBeads,32,3,5,1.5,1,1);
            % 
            % figure
            % quiver(curUnmatchedBeads(:,1), curUnmatchedBeads(:,2),distNewMatches(:,1), distNewMatches(:,2),0)
            % 
            % 
            % tptGUI = TPTGUI({{curUnmatchedRef},{curUnmatchedBeads}}, finaltptParams{1});
            % %update tptParameter from this
            % %waitfor(tptGUI)
            % while(tptGUI.closenum ~= 1)
            %     pause(1)
            % end
            % 
            % finaltptParams2 = tptGUI.tptParamFinal;
