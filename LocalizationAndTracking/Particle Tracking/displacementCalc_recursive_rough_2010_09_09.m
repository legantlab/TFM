function [matchedCoordinates]=displacementCalc_recursive_rough_2010_09_09(reference,deformed,numNeighbors,numVectorsRef,numVectorsDef,tol,numProcessors,jm)
...
[matchedCoordinates]=displacementCalc_2010_09_09(reference,deformed,numNeighbors,numVectorsRef,numVectorsDef,tol,numProcessors,jm); %initial matching
matchedCoordinates_New=matchedCoordinates; %store all matches

while(length(matchedCoordinates_New)/length(matchedCoordinates)>.2&&length(setdiff(reference(:,2:4),matchedCoordinates(:,2:4),'rows'))>numNeighbors&&length(setdiff(deformed(:,2:4),matchedCoordinates(:,6:8),'rows'))>numNeighbors)
    ...
    [matchedCoordinates_New]=displacementCalc_2010_09_09([[1:1:length(reference_new)]',reference_new],[[1:1:length(deformed_new)]',deformed_new],numNeighbors,numVectorsRef,numVectorsDef,tol,numProcessors,jm);
