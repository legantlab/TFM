...
path = 'X:\Max\2021_07_07_F127GellCharacterization\First Half\Test2'; % input folder
...
z_range = 516:3:540; % user-defined range where channel looks ideal, ~15 images
...
%% Gathering Rotation Angles from Selected Layers (7-13-2021, 7-21-21 -- NP)
...
for i = z_range
    ...
    % conditional for layers that have small bubbles or other non-channel
    % isolated objects
    CC = bwconncomp(BW3{k}); 
    if CC.NumObjects ~= 2
        continue
...
avg_ang = 90 - mean(rmoutliers(stored_angs)); % average rotation value

%% Rotating and Isolating Channel in Selected Layers
...
for j = 1:length(z_range)
    ...
    % rotating image
    img_rotated = imrotate(BW3{j}, avg_ang); % orients channel vertically
    % after rotation
    reg_data_rot = [];
    ...
    %channel_dims = [channel_width, channel_height]; % storing channel dimensions, WxL
    ...
    x_adj(j) = size(img_rotated, 1)/2 - cent_x; % adjustment in x to center of image
    y_adj(j) = size(img_rotated,2)/2 - cent_y; % adjustment in y
...
for i = 3:3:z_end
    ...
    img_rot{l} = imrotate(img_initial{l}, avg_ang);
...
%%
% img = imbinarize(img_trans{178},'adaptive','ForegroundPolarity','bright','Sensitivity',0.6);
% figure(1)
% imshow(img)
% hold on 
% plot([0 max(sizes)], [max(sizes)/2 max(sizes)/2], 'b',...
%     [max(sizes)/2 max(sizes)/2], [0 max(sizes)], 'b', 'LineWidth', 2)
% hold off
...
%% Edge Detection Test
img_temp = imread(imgfile, 516); % read in DIC images
...
img = imfill(BWoutline, 'holes');
imshow(img)
%imwrite(im2uint16(BWoutline),'segout6.png')
%%
