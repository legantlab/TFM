...
path = 'X:\Max\2021_07_07_F127GellCharacterization\First Half\Test 4\output'; % input folder
...
for ii = 1:length(tif_paths)
    ...
    % previous edge detection finds which z-values correspond to the most
    % ideal channel structures in the total z-stack, then saves those
    % values for later rotation and translation analysis
 ...
 %%
        ...
        for aa = 2:size(sorted_channel2,1)
            if sorted_channel2(aa,2) - sorted_channel2(aa-1,2) > 50 ...
                ...
                mm = mm+1;
                continue % identifies the inner edges of the binary image -- these 
                % are the outer walls of the channel
            end
        end
        ...
        for bb = 2:size(boundaries2,1)
            for cc = 1:size(BW2,1)
                if cc >= boundaries2(bb,2) && cc <= boundaries2(bb,3)
                    BW2(boundaries2(bb,1),cc) = 1;
                elseif cc < boundaries2(bb,2)
                    BW2(boundaries2(bb,1),cc) = 0;
                elseif cc > boundaries2(bb,3)
                    BW2(boundaries2(bb,1),cc) = 0;
                end
            end 
            
        end % fills in channel and makes the previous boundaries 0
        ...
        %% Sum Test -- Unreliable
        % the sum_across attempt is pretty unreliable, while the
        % stats.Centroid property isn't perfect, it's not super inaccurate
        
%         sum_across = sum(BW2,2); % sums across to find the constriction of the channel
%         
%         sum_across(sum_across <= 60) = 1e3;
%         upper_limit = round(img_size(1)/10);
%         sum_across(1:upper_limit) = 1e3;
%         bottom_limit = round(7*img_size(1) / 8);
%         sum_across(bottom_limit:end) = 1e3;
%         % filters for extremes of the channel
%         
%         x_occupied = [];
%         [cent_val(k), ind(k)] = min(sum_across); % finds center of constriction
%         ind(ind > img_size(1)) = [];
%         ind(ind < 1) = [];
%         
%         for bbb = 1:size(BW2,1)
%             if BW2(ind, bbb) == 1
%                 x_occupied = [x_occupied, bbb];
%             end
%         end
%         x_cent_val(k) = median(x_occupied); % center of thinnest part of the channel
        
        %% Reliable Translation Amounts
        
        x_shift(k) = img_center(1) - stats.Centroid(2));
        x_cent(k) = stats.Centroid(2);
        y_shift(k) = img_center(2) - stats.Centroid(1);
        y_cent(k) = stats.Centroid(1);
        k = k+1;
    end

%%
x_shift(isnan(x_shift)) = []; % removes invalid shift amounts
average_angle = mean(angle_val);
average_x_shift = mean(x_shift);
x_shift_comp = mean(rmoutliers(x_shift));
x_shift_corr = average_x_shift - x_shift_comp;
% different x_shift tests to see which aligns the best

average_y_shift = mean(rmoutliers(y_shift));
...
%% Actual Translation
    ...
    for iii = 3:3:z_end
        ...
        img_trans{l} = imtranslate(img_rot{l}, [x_shift_comp, -average_y_shift]);
...
imgm = imbinarize(img_trans{154},'adaptive','ForegroundPolarity','bright','Sensitivity',0.6);
figure(1)

imshow(imgm)
hold on
plot([0 size(imgm,1)], [size(imgm,1)/2 size(imgm,1)/2], [size(imgm,1)/2 size(imgm,1)/2],...
    [0 size(imgm,1)], 'LineWidth', 2)
hold off
