...
for i = 30:3:z_end - 10
    ...
    img_rotated = imrotate(BW2, -angle+90); % orients channel vertically
    img_rotated_actual = imrotate(img_temp_bin, -angle + 90); % not binary
    stored_imgs{k} = img_rotated;
    
    % after rotation
    reg_data_rot = [];
    [reg_data_rot(:,2), reg_data_rot(:,1)] = find(img_rotated);
    % finds coordinates of channel after rotation
    rotated_in_channel{k} = reg_data_rot;
    ...
    channel_dims{k} = [channel_width, channel_height]; % storing channel dimensions, WxL
    
    cent_x = channel_width / 2 + left_channel; % center of channel in x
    
    img_cropped = imcrop(img_rotated, [left_channel bottom_channel channel_width channel_height]);
    % cropping rotated image to isolate the channel
    stored_crops{k} = img_cropped;
    ...
    x_adj(k) = size(img_rotated, 1)/2 - cent_x; % adjustment in x to center of image
    y_adj(k) = size(img_rotated,2)/2 - cent_y; % adjustment in y
    
    img_centered = imtranslate(img_rotated_actual, [x_adj(k), y_adj(k)]);
...
setTag(t,tagStruct)
write(t,squeeze(im2uint16(stored_centered_imgs{1})));

for k = 2:length(stored_centered_imgs)
    writeDirectory(t);
    setTag(t,tagStruct)
    write(t,squeeze(im2uint16(stored_centered_imgs{k})));
end
