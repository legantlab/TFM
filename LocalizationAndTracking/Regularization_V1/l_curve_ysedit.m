function [reg_corner,rho,eta,reg_param] = l_curve(U,sm,b,method,fpseudoInv)
...
if (strcmp(method,'L1'))
  ...
  %reg_param = logspace(-1,0,npoints)';
  ...
  marker = '-'; txt = 'L1.';
  solutionMagCondition = abs(log10(eta/norm(fpseudoInv,1)))<.5;
  eta = eta(solutionMagCondition);
  rho = rho(solutionMagCondition);
  reg_param = reg_param(solutionMagCondition);
...
elseif (strncmp(method,'Tikh',4) | strncmp(method,'tikh',4))
  ...
  if (m > n & beta2 > 0), rho = sqrt(rho.^2 + beta2); end
  marker = '-'; txt = 'Tikh.';
  %solutionMagCondition = abs(log10(norm(fpseudoInv))-log10(eta))<.5;
  %eta = eta(solutionMagCondition);
  %rho = rho(solutionMagCondition);
  %reg_param = reg_param(solutionMagCondition);
else
...
if (locate)
  [reg_corner,rho_c,eta_c] = l_corner_ysedit(rho,eta,reg_param,U,sm,b,method);
end
% Make plot.
...
if locate
  ...
  loglog([min(rho),rho_c],[eta_c,eta_c],':r',...
         [rho_c,rho_c],[min(eta),eta_c],':r')
