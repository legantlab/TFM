function [x_lambda,rho,eta] = tikhonov(U,s,V,b,lambda,method,x_0)
...
if (ps==1)
    ...
    for i=1:ll
        ...
        eta(i) = norm(x_lambda(:,i),normOrder);
    end
...
elseif (m>=n)
    ...
    gamma2 = (s(:,1)./s(:,2)).^2;
    if (nargin==7), omega = V\x_0; omega = omega(1:p); end
    if (p==n)
        x0 = zeros(n,1);
    else
    ...
    gamma2 = (s(:,1)./s(:,2)).^2;
    if (nargin==7), error('x_0 not allowed'), end
    if (p==m)
        x0 = zeros(n,1);
    else
