...
fold = 'C:\Users\maxho\Desktop\6_22_2023_Swelling\CompiledDisps';
...
for i = 1:length(allMatches)   
    ...
    % Find optimum shift for each swelling data set
        ...
        curMatch = matches{1};
        curMatch(:,3) = curMatch(:,3);
        curMatch(:,6) = curMatch(:,6);
        curDisp = curMatch(:,4:6) - curMatch(:,1:3);
        %Perform automated Alignment
        [finalElements,finalDisplacements] = alignChannels([curMatch(:,1:3), curDisp],elemCents2D,0);
        ...
        newMatch = {[finalElements(:,1) + finalDisplacements(:,1), finalElements(:,2) + finalDisplacements(:,2) ...
            , finalElements(:,3) + finalDisplacements(:,3), ...
            finalElements(:,1), finalElements(:,2), finalElements(:,3)]};

        curInterp = interpDisps(newMatch,elemCents2D);   
...
figure
quiver3(elemCents2D(:,1), elemCents2D(:,2),elemCents2D(:,3),avgXDisp,avgYDisp, avgZDisp,0)
hold on
scatter3(elemCents2D(:,1), elemCents2D(:,2),elemCents2D(:,3),'b')
...
%% Save
save('averagedSwellingProfile7x40_expDerivedProfilePixel.mat','averageDisplacement','elemCents2D')
