function [finalLocalizations, trans] = alignOriginStraight(referenceLocs,plotFlag)
...
%and z is the top of the substrate. This function works with straight
%channel feature rather than confinements. - MH 12/18/2023
ImgDim = [max(referenceLocs(:,1)) - min(referenceLocs(:,1)),max(referenceLocs(:,2)) - min(referenceLocs(:,2)), ...
    max(referenceLocs(:,3)) - min(referenceLocs(:,3))];%this might need to be flipped
%Align X
datafiltUp = referenceLocs(:,2)>=quantile(referenceLocs(:,2),.75);

%Invert for guassian fit. 
dataupFitVal = [referenceLocs(datafiltUp,1),-1*referenceLocs(datafiltUp,3)];
...
datafiltDown = referenceLocs(:,2) <= quantile(referenceLocs(:,2),.25);


%Invert for guassian fit. 
datadownFitVal = [referenceLocs(datafiltDown,1),-1*referenceLocs(datafiltDown,3)];
%fix z to zero
...
xCorLocalizations = referenceLocs;
...
%% Perform Y Alignment
%In this case, we just want to move the axis to 0 given we don't have a
%constriction. Maybe plot median value of y and subtract? 

yMedian = median(xzCorLocalizations(:,2));
...
xzyCorLocalizations(:,2) = xzyCorLocalizations(:,2) - yMedian;
